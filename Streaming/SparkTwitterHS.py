...
'''
Class: CS483 Spring 2020

Authors: Evan Bariquit and Garth Scheck 

Application: Tweet statistics using Spark Streaming 
'''
...
def processWords(time, rdd):
    ...
    try:
        ...
        df=df.filter(~df.word.like('[%') & ~df.word.like('{%') & ~df.word.like('') & ~df.word.like('and') )
...
# send data to MySQL for dashboard   
